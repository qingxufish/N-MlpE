...
import pickle
import copy
from .attention_packge import ConvR_attention
from .attention_packge import attention_layer


class bais_infer(BaseModel):
    def __init__(self, config):
        super(bais_infer, self).__init__(config)
        self.device = config.get('device')
        self.entity_cnt = config.get('entity_cnt')
        self.relation_cnt = config.get('relation_cnt')
        kwargs = config.get('Bias_params')
    ...
    def single_infer2entity(self, n_n, n_n_r, n_h, n_r):  # neighbour, neighbour relation, head entity, predict relation
        ...
        x = x.view(n_size, -1)
        x = self.fc(x)
        b = torch.mm(x, self.E.weight.transpose(1, 0))
        return b.view(n_size, -1)
...
class ConvR(BaseModel):
    def __init__(self, config):
        ...
        self.entity_cnt = config.get('entity_cnt')
        self.relation_cnt = config.get('relation_cnt')
        kwargs = config.get('ConvR_params')
        self.conv_out_channels = kwargs.get('conv_out_channels')
        self.reshape = kwargs.get('reshape')
        self.kernel_size = kwargs.get('conv_kernel_size')
        self.stride = kwargs.get('stride')
    ...
    def forward(self, batch_h, batch_r, batch_t=None):
        ...
        y = torch.sigmoid(x)
        return self.loss(y, batch_t), y, x
...
class ConvE(BaseModel):
    def __init__(self, config):
        ...
        self.entity_cnt = config.get('entity_cnt')
        self.relation_cnt = config.get('relation_cnt')
        kwargs = config.get('ConvModel_params')
        self.emb_dim = kwargs.get('emb_dim')
        self.E = torch.nn.Embedding(self.entity_cnt, self.emb_dim)
    ...
    def predict(self, batch_h, batch_r):
        ...
        batch_size = batch_h.size(0)
        e1 = self.unitized(self.E(batch_h)).view(-1, 1, *self.reshape)
        r = self.unitized(self.R(batch_r)).view(-1, 1, *self.reshape)
...
class multi_infer(BaseModel):
    def __init__(self, config, single_infer_model):
        ...
        self.eval_ConvModel = 1
        self.single_infer = single_infer_model
        self.ConvModel = ConvE(config)
    ...
    def forward(self, batch_h, batch_r, batch_t=None):
        x = self.ConvModel.predict(batch_h, batch_r)
        if self.eval_ConvModel:
            y = torch.sigmoid(x)
        else:
            b = self.batch_multi_infer_model(batch_h, batch_r, batch_t)
            x += b
            y = torch.sigmoid(x)
        return self.loss(y, batch_t), y


class bais_ConvE(BaseModel):
    def __init__(self, config):
        super(bais_ConvE, self).__init__(config)
        self.device = config.get('device')
        self.entity_cnt = config.get('entity_cnt')
        self.relation_cnt = config.get('relation_cnt')

        self.bais_infer = bais_infer(config)
        self.multi_infer = multi_infer(config, self.bais_infer)
