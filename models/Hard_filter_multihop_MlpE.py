...
from .attention_packge import SA_attention, ALL_attention
...
class Hard_filter_multihop_MlpE(BaseModel):
    ...
    def pre_train_aggregate_message_by_group(self, n_n, n_n_r, n_h, n_r, MhE_interval):
        all_attention_value = self.attention(n_n, n_n_r, n_h, n_r)
        aggregate_message = self.multi_hop_message(n_n=n_n, n_n_r=n_n_r, n_h=n_h, n_r=n_r)
        all_aggregated_message = torch.tensor([]).to(self.device)
        e_ind = 0
        for ind, interval in enumerate(MhE_interval):
            s_ind = e_ind
            e_ind = s_ind + int(interval)
            softmax_attention = self.softmax(all_attention_value[s_ind:e_ind, :])
            group_message = aggregate_message[s_ind:e_ind, :]

            aggregated_message = torch.mm(softmax_attention.T, group_message).view(1, -1)
            all_aggregated_message = torch.cat((all_aggregated_message,
                                            aggregated_message),
                                           dim=0)
        return all_aggregated_message

    def pre_train_batch_find_neighbour_and_rel(self,  batch_h, batch_r, batch_t):
        MhE_n_n = torch.tensor([]).to(self.device)
        MhE_n_n_r = torch.tensor([]).to(self.device)
        MhE_n_h = torch.tensor([]).to(self.device)
        MhE_n_r = torch.tensor([]).to(self.device)
        ...
        e1 = self.pre_train_aggregate_message_by_group(n_n=MhE_n_n,
                                             n_n_r=MhE_n_n_r,
                                             n_h=MhE_n_h,
                                             n_r=MhE_n_r,
                                             MhE_interval=MhE_interval)
    ...
    def pre_train_attention(self,batch_h, batch_r, batch_t):
        m1 = self.pre_train_batch_find_neighbour_and_rel(batch_h, batch_r, batch_t)
