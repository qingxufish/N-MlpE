...
class DIS_ConvE(BaseModel):
    def __init__(self, config):
        super(DIS_ConvE, self).__init__(config)
    ...
    def forward(self, batch_h, batch_r, batch_t=None):
        ...
        expand_x = x.unsqueeze(2).repeat(1, 1, self.entity_cnt).to(torch.float16)
        expand_entity_embedding = self.E.weight.transpose(1, 0).unsqueeze(0).repeat(batch_size, 1, 1).to(torch.float16)
        z = torch.norm(expand_entity_embedding - expand_x, p=2, dim=1).to(torch.float32)
        # z = z + self.b.expand_as(z)
        y = torch.exp(-torch.pow(torch.abs(z), self.c))
