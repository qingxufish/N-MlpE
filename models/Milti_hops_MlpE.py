...
import pickle
import copy
from .attention_packge import abs_attention
...
class single_infer(BaseModel):
    def __init__(self, config):
        ...
        self.R = torch.nn.Embedding(self.relation_cnt*2+1, self.relation_dim)  # 反向为另一种关系
        self.init()

        self.hidden_drop = torch.nn.Dropout(kwargs.get('hidden_dropout'))

        self.bn0_1 = torch.nn.BatchNorm1d(1)
        self.bn0_2 = torch.nn.BatchNorm1d(1)
        self.bn0_3 = torch.nn.BatchNorm1d(1)
        self.bn0_4 = torch.nn.BatchNorm1d(1)
        self.bn0_5 = torch.nn.BatchNorm1d(1)
        self.bn0_6 = torch.nn.BatchNorm1d(1)

        self.bn2 = torch.nn.BatchNorm1d(1)

        self.register_parameter('c', Parameter(torch.zeros(1)))

        self.fc_m1r1 = torch.nn.Linear(self.entity_dim+self.relation_dim, self.entity_dim*self.relation_dim)
        self.fc_m1e1 = torch.nn.Linear(self.entity_dim+self.entity_dim, self.entity_dim*self.entity_dim)
        self.fc_m1r = torch.nn.Linear(self.entity_dim+self.relation_dim, self.entity_dim*self.relation_dim)
        self.fc_r1e1 = torch.nn.Linear(self.relation_dim+self.entity_dim, self.relation_dim*self.entity_dim)
        self.fc_r1r = torch.nn.Linear(self.relation_dim+self.relation_dim, self.relation_dim*self.relation_dim)
        self.fc_e1r = torch.nn.Linear(self.entity_dim+self.relation_dim, self.entity_dim*self.relation_dim)

        self.decoder = torch.nn.Linear(self.entity_dim*self.relation_dim+
    ...
    def single_infer2entity(self, n_n, n_n_r, n_h, n_r):  # neighbour, neighbour relation, head entity, predict relation
        batch_size = n_r.size(0)
        m1 = self.unitized(self.E(n_n.view(-1)))
        r1 = self.unitized(self.R(n_n_r.view(-1)))
        e1 = self.unitized(self.E(n_h.view(-1)))
        r = self.unitized(self.R(n_r.view(-1)))
        stacked_m1r1 = torch.cat([m1, r1], dim=1).view(batch_size, 1, -1)
        stacked_m1e1 = torch.cat([m1, e1], dim=1).view(batch_size, 1, -1)
        stacked_m1r = torch.cat([m1, r], dim=1).view(batch_size, 1, -1)
        stacked_r1e1 = torch.cat([r1, e1], dim=1).view(batch_size, 1, -1)
        stacked_r1r = torch.cat([r1, r], dim=1).view(batch_size, 1, -1)
        stacked_e1r = torch.cat([e1, r], dim=1).view(batch_size, 1, -1)

        stacked_m1r1 = self.bn0_1(stacked_m1r1)
        stacked_m1e1 = self.bn0_2(stacked_m1e1)
        stacked_m1r = self.bn0_3(stacked_m1r)
        stacked_r1e1 = self.bn0_4(stacked_r1e1)
        stacked_r1r = self.bn0_5(stacked_r1r)
        stacked_e1r = self.bn0_6(stacked_e1r)

        m1r1 = self.fc_m1r1(stacked_m1r1)
        m1e1 = self.fc_m1r1(stacked_m1e1)
        m1r = self.fc_m1r1(stacked_m1r)
        r1e1 = self.fc_m1r1(stacked_r1e1)
        r1r = self.fc_m1r1(stacked_r1r)
        e1r = self.fc_m1r1(stacked_e1r)

        feature_vector = torch.cat([m1r1, m1e1, m1r, r1e1, r1r, e1r], dim=1).view(batch_size, 1, -1)
        x = F.leaky_relu(feature_vector, 0.1)
        x = self.decoder(x)
        x = self.bn2(x)
    ...
    def pre_train(self, batch_h=None, batch_r=None, batch_n=None, batch_n_r=None, batch_t=None):
        batch_size = batch_n.size(0)
        m1 = self.unitized(self.E(batch_n.view(-1)))
        r1 = self.unitized(self.R(batch_n_r.view(-1)))
        e1 = self.unitized(self.E(batch_h.view(-1)))
        r = self.unitized(self.R(batch_r.view(-1)))
        stacked_m1r1 = torch.cat([m1, r1], dim=1).view(batch_size, 1, -1)
        stacked_m1e1 = torch.cat([m1, e1], dim=1).view(batch_size, 1, -1)
        stacked_m1r = torch.cat([m1, r], dim=1).view(batch_size, 1, -1)
        stacked_r1e1 = torch.cat([r1, e1], dim=1).view(batch_size, 1, -1)
...
class multi_infer(BaseModel):
    def __init__(self, config, single_infer_model):
        ...
        self.attention = abs_attention(config)
    ...
    def score_based_infer(self, n_n, n_r, h, r):  # 用于合并所有路径得到的实体
        ...
        predict_attention = self.attention(n_r, r)
        predict_y = predict_y*predict_attention.view(-1, 1).expand_as(predict_y)
        # y = self.logic_semantic_decision(predict_attention, infer_confidence, predict_x)
        y, _ = torch.max(predict_y, dim=0)
        return y.view(1, -1)
    ...
    def batch_multi_infer_model(self, batch_h, batch_r, batch_t):  # multi predict train
        ...
        for batch_ind, head_id in enumerate(batch_h):
            ...
            batch_y = torch.cat((batch_y, self.score_based_infer(batch_n, batch_n_r, batch_h_, batch_r_)), dim=0)
...
class Multi_hops_MlpE(BaseModel):
    def __init__(self, config):
        super(Multi_hops_MlpE, self).__init__(config)
        self.device = config.get('device')
