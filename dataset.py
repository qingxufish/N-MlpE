...
from utils import load_triples, load_ids, load_double_triples, load_infer_triples
...
class Dataset:
    ...
    def read_double_triplets_train(self):
        logging.info(' Loading training data '.center(100, '-'))
        return load_double_triples(os.path.join(self.path, 'data', self.name, 'double_triplets.tsv'))
    ...
    def read_infer_triplets_train(self):
        logging.info(' Loading realtion id '.center(100, '-'))
        return load_infer_triples(os.path.join(self.path, 'data', self.name, 'infer_triplets.tsv'))
